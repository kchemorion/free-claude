...
class GitIntegration {
    constructor(claudeAPI, maxDiffSize = 1000, maxFileSize = 1000000) {
    ...
    async getRepoContext(forceRefresh = false) {
    ...
    async getLastCommit() {
        const { stdout } = await execAsync('git log -1 --pretty=format:"%H%n%an%n%ad%n%s"', { cwd: this.workspaceRoot });
    ...
    async analyzeComplexity() {
        ...
        const { stdout: changes } = await execAsync('git log --format=format: --name-only | sort | uniq -c | sort -rn | head -n 10', { cwd: this.workspaceRoot });
        return {
    ...
    async findIndirectDependencies(packageJson) {
        const nodeModulesPath = path.join(this.workspaceRoot, 'node_modules');
        const indirect = new Set();
        const checkDependencies = async (deps) => {
            for (const dep of Object.keys(deps)) {
                try {
                    const depPackageJson = JSON.parse(await fs.readFile(path.join(nodeModulesPath, dep, 'package.json'), 'utf-8'));
...
[suggestion (if applicable)]`;
    ...
    parseCodeReview(review) {
        ...
        for (const line of lines) {
            const match = line.match(/\[(info|warning|error)\]\s*\[(security|performance|maintainability|bug|style)\]\s*([^:]+):(\d+):\s*(.+?)(?:\s*\[suggestion:\s*(.+)\])?$/);
            if (match) {
...
7. Appropriate labels based on the type of changes`;
    ...
    async getCommitsSinceBase(baseBranch) {
        const { stdout } = await execAsync(`git log ${baseBranch}..HEAD --pretty=format:"%H%n%s%n%an%n%ad"`, { cwd: this.workspaceRoot });
        const commits = [];
        const lines = stdout.split('\n');
...
5. Ensure complete coverage of the codebase`;
    ...
    async getGitBlame(file) {
        const { stdout } = await execAsync(`git blame --line-porcelain ${file}`, { cwd: this.workspaceRoot });
        const lines = stdout.split('\n');
        const blame = {
            hash: '',
            author: '',
...
GitIntegration.CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
